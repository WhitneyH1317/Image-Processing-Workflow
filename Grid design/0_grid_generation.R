...
# view camera points
mapview(mesq_pts, col.regions = "green") + mapview(mes) + mapview(ref_pts, col.regions = "green") + mapview(reforma) 

# count cameras
nrow(ref_pts) + nrow(mesq_pts)
save(ref_pts, mesq_pts, file = "hunting_lease_grids.rda")
# save in workingName# save in working directory of choice
#st_write(ref_pts, "output/reforma_camera_grid_locs_1000.shp",
#         append = FALSE)
#st_write(mesq_pts, "output/muria_camera_grid_locs_1000.shp",
#         append = FALSE)

###### BROADER GRID ######
# load data
ef<- st_read("data/EF GIS Layers/Properties.shp")
mapview(ef)
# process data
